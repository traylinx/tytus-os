...
describe('loadFeaturedApps', () => {
  ...
  it('preserves traylinx raw GitHub manifest URLs', async () => {
    ...
    expect(list[0].manifestUrl).toBe(
      'https://raw.githubusercontent.com/traylinx/tytus-app-atomek/v0.4.29/tytus-app.json',
    );
  });
...
describe('FEATURED_APPS hardcoded baseline', () => {
  ...
  it('includes restored Text Editor and Markdown Editor while omitting Code Editor', () => {
    ...
    expect(FEATURED_APPS.find((a) => a.id === 'atomek')?.manifestUrl).toBe('https://raw.githubusercontent.com/traylinx/tytus-app-atomek/v0.4.29/tytus-app.json');
