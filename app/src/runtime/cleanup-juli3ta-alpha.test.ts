...
describe('upgradeJuli3taGatewayFixIfStale', () => {
  it('upgrades existing standalone JULI3TA rows below v0.3.26 to the chat-safe tag', async () => {
    ...
    expect(report.reason).toMatch(/0\.3\.2 to 0\.3\.26/);
    const [row] = await listInstalledApps(db);
    expect(row?.manifest.version).toBe('0.3.26');
    expect(row?.entryUrl).toBe(
      'https://cdn.jsdelivr.net/gh/traylinx/tytus-app-juli3ta@juli3ta-0.3.26/dist/index.js',
    );
    expect(row?.manifestUrl).toBe(
      'https://cdn.jsdelivr.net/gh/traylinx/tytus-app-juli3ta@juli3ta-0.3.26/tytus-app.json',
    );
  });

  it('keeps current standalone JULI3TA rows unchanged', async () => {
    await insertInstalledApp(db, alphaRow('0.3.26'));

    const report = await upgradeJuli3taGatewayFixIfStale(db);

    expect(report.upgraded).toBe(false);
    expect(report.reason).toBe('version 0.3.26 is current');
    const [row] = await listInstalledApps(db);
    expect(row?.manifest.version).toBe('0.3.26');
