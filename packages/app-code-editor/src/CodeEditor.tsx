...
// ============================================================
//
// Lifted from app/src/apps/CodeEditor.tsx as part of
// SPRINT-TYTUS-APP-SYSTEM-V1 Phase 5. Shell-internal hooks
// (`useFileSystem`) replaced with async `host.fs.*` calls. The inlined
// FileTree was rewritten to lazy-load each folder's children via
// `host.fs.list(parentId)` and cache them per-id, with refresh after
// any write/rename/delete (or `host.fs.watch` if available).
...
import type { HostClient, FsChangeEvent } from '@tytus/host-api';

// Inferred from `host.fs.list` to keep the package dep on @tytus/host-api
// only. Mirror of `FileNode` from @tytus/contracts (re-exported through
// FsApi.list's return) — locked to host-api v1.0.0.
type FileNode = Awaited<ReturnType<HostClient['fs']['list']>>[number];
import {
  Code2,
  X,
  Plus,
...
// ---- File tree component (async, host-api driven) ----
//
// Children are loaded on demand via `host.fs.list(parentId)` and cached
// in a `Map<id, FileNode[]>` shared across the recursive subtree. When
// the user expands a folder we hydrate its entry; on any external change
// (host.fs.watch) or after a refresh-trigger we re-list affected nodes.
...
function FileTree({
  ...
  // Trigger initial load once per parent.
  useEffect(() => {
    if (!childrenCache.has(parentId)) {
      void onLoadChildren(parentId);
    }
  }, [parentId, childrenCache, onLoadChildren]);

  const children = (childrenCache.get(parentId) ?? []).filter(
    (c) => !c.name.startsWith('.'),
  );
      ...
      {children.map((child) => (
        <div key={child.id}>
          <div
            ...
            onClick={() => {
              if (child.isDirectory) onToggle(child.id);
          ...
          {child.isDirectory && expanded.has(child.id) && (
            <FileTree
              parentId={child.id}
              host={host}
...
export function CodeEditor({ host }: Props) {
  ...
  // Tree state lifted to the editor so refreshes after writes can
  // surgically re-list a single parent without remounting the whole tree.
  // The cache + expanded set are passed by reference to FileTree subtrees;
  // setVersion bumps to force re-render after async mutation.
  ...
  const loadChildren = useCallback(
    async (parentId: string) => {
      try {
        const list = await host.fs.list(parentId);
        childrenCacheRef.current.set(parentId, list);
        bumpTree();
      } catch (err) {
        flashStatus(`Load failed: ${(err as Error).message}`, 3000);
      }
    },
    [host, bumpTree, flashStatus],
  );

  const refreshParent = useCallback(
    async (parentId: string) => {
      // Force re-list (bypassing cache check) for an already-known parent.
      try {
        const list = await host.fs.list(parentId);
        childrenCacheRef.current.set(parentId, list);
        bumpTree();
      } catch {
        // Swallow — sidebar will retry next interaction.
      }
    },
    [host, bumpTree],
  );
  ...
  // Resolve the user's documents folder once on mount and treat it as the
  // sidebar root. (Legacy code walked /home/user; the host-api equivalent
  // is `ensureUserFolder('documents')` which is the only path the
  // `vfs.user.documents` permission grants us read access to.)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled) return;
        flashStatus(`Folder unavailable: ${(err as Error).message}`, 3000);
      }
    })();
    return () => {
      cancelled = true;
  ...
  // Subscribe to fs changes so the tree refreshes when other apps mutate
  // the same folders. `host.fs.watch` returns a disposer.
  useEffect(() => {
    if (!rootId) return;
    const disposer = host.fs.watch(
      rootId,
      (event: FsChangeEvent) => {
        // Re-list the affected parent if we have it cached.
        if (childrenCacheRef.current.has(event.parentId)) {
          void refreshParent(event.parentId);
        }
      },
      { recursive: true },
    );
    return () => disposer();
  }, [host, rootId, refreshParent]);
  ...
  const saveFile = useCallback(async () => {
    ...
    try {
      if (activeTab.fileNodeId) {
        await host.fs.write(activeTab.fileNodeId, activeTab.content);
        setTabs((prev) =>
          prev.map((t) =>
            t.id === activeTab.id ? { ...t, isDirty: false } : t,
          ),
        );
        flashStatus(`Saved ${activeTab.name}`);
      } else {
        // No backing file — drop into Documents.
        const docs = await host.fs.ensureUserFolder('documents');
        ...
        setTabs((prev) =>
          prev.map((t) =>
            t.id === activeTab.id
              ? { ...t, fileNodeId: newId, isDirty: false }
              : t,
          ),
        );
        // Refresh sidebar so the new file appears.
        void refreshParent(docs);
        flashStatus(`Saved to Documents/${activeTab.name}`);
      }
    } catch (err) {
      flashStatus(`Save failed: ${(err as Error).message}`, 3000);
    }
  }, [activeTab, host, refreshParent, flashStatus]);
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={newFile}
          className="flex items-center gap-1 px-2 py-1 rounded-sm hover:bg-[var(--bg-hover)] transition-colors"
          title="New File"
        >
          <Plus size={14} />
      ...
      {showFindBar && (
        ...
        >
          ...
          <button
            ...
            className="text-xs px-2 py-0.5 rounded-sm hover:bg-[var(--bg-hover)]"
          >
            Esc
          </button>
          {findQuery && (
          ...
          >
            ...
            {rootId && (
              <FileTree
                ...
                onOpen={(id, name) => void openFile(id, name)}
              />
            )}
          </div>
        )}
          ...
          {activeTab ? (
            <>
              <div className="flex-1 flex overflow-hidden relative">
                ...
                >
                  {lineNumbers.map((n) => (
                    <div key={n} className="pr-3" style={{ height: 20 }}>
                      {n}
                    </div>
                  ))}
                </div>
              ...
              {/* Status Bar */}
              <div
                className="flex items-center justify-between px-3 py-0.5 shrink-0"
                style={{
                  background: 'var(--bg-titlebar)',
              ...
              >
                ...
                <span
                  ...
                  style={{ color: 'var(--text-secondary)' }}
                >
                  {activeTab.language} | UTF-8 | {activeTab.content.length}{' '}
                  chars
                </span>
              </div>
            </>
