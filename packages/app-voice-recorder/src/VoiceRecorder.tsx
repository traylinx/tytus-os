// ============================================================
// Voice Recorder — workspace-package edition.
// ============================================================
//
// Lifted from app/src/apps/VoiceRecorder.tsx as part of W3 of the
// apps-platform sprint. Behaviour parity with the in-tree app:
// real microphone capture via MediaRecorder + AnalyserNode-driven
// waveform + SQLite-backed persistence. Differences:
//
//   - Mic acquisition goes through `host.media.requestMicrophone()`
//     instead of `navigator.mediaDevices.getUserMedia` + the local
//     mime-probe ladder. The host already runs the same probe and
//     returns the live stream + the best-supported mime type in one
//     call.
//
//   - Persistence goes through the bound `AppDb` (per-app SQLite,
//     prefix-guarded) passed in by `bootVoiceRecorder(env)`. The
//     repo file owns all the SQL and is unaware of the host.
//
//   - The legacy localStorage drain runs shell-side before this
//     component mounts; it does NOT live here anymore (workspace
//     packages can't read un-prefixed tables).
...
} from './repo/voiceRecordings';

// ---- Types ----
// Re-exported alias — keeps the public type name stable for any
// future external consumer that imported `VoiceRecording` from this
// file historically.
...
export function VoiceRecorder({ db, host }: VoiceRecorderProps) {
  // Hydrated async from SQLite. Starts empty; the AppDb's lazy resolve
  // means first paint isn't blocked by db init.
  ...
  // Initial hydrate from SQLite. Workspace edition: no localStorage
  // migration here — the shell shim runs the legacy drain before
  // this package boots.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const recs = await listRecordings(db);
        if (!cancelled) setRecordings(recs);
      } catch (e) {
        if (!cancelled) setError((e as Error).message || 'Could not load recordings.');
      }
    })();
  ...
  const startRecording = async () => {
    setError(null);
    try {
      // Host owns the mic-probe ladder. Returns the live stream plus
      // the best-supported MediaRecorder mime type in one call.
      const { stream, mimeType } = await host.media.requestMicrophone();
  ...
  const deleteRecording = (id: string) => {
    ...
    void deleteRecordingRow(db, id).catch((e: unknown) => console.warn('Recording delete failed:', e));
  };
  ...
  const renameRecording = (id: string, name: string) => {
    ...
    void updateRecordingName(db, id, cleaned).catch((e: unknown) => console.warn('Recording rename failed:', e));
  };
